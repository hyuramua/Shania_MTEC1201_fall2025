...
// for loop that allows hearts to fall 

for (let i=0; i<hearts.length; i++) {
    hearts[i].display();
    hearts[i].fall();
}
// loop that deletes hearts after falling off screen for smoother run
for (let i=0; i<hearts.length; i++) {
  if (hearts[i].y > height + 20 )  {
    
// splice removes the hearts as the fall by erasing it when it is off screen, number for how many you'd want to delete
...
// push to push in new heart object
